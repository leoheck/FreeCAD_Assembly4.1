...
if os.path.isfile(git_head_filepath):
    with open(git_head_filepath) as f:
        git_branch = f.readline().split(" ")[1].split("/")[-1].replace("\n", "")
    ...
    def Initialize(self):
        ...
        if FCver[0] == "0" and FCver[1] == "22":
            ...
            if isinstance(git, int) and git > 35594:
                ...
                App.Console.PrintWarning(
                    "is not backward compatible with FreeCAD v0.21 and earlier\n"
                )
                App.Console.PrintWarning(
                    "It is rather suggested to use the stable FreeCAD v0.21 branch\n"
                )
        ...
        App.Console.PrintMessage(
            "Initializing Assembly4 workbench {}{}... ".format(Asm4_version, git_branch)
        )
