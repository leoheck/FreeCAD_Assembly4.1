...
class importDatumCmd:
    ...
    def Activated(self):
        ...
        if selTree:
            ...
            if (
                len(selection) == 2
                and selection[0].getParentGeoFeatureGroup() == rootContainer
            ):
                confirm = False
                ...
                if selDatum.MapMode == "Deactivated":
                    message = (
                        "This will superimpose "
                        + Asm4.labelName(selDatum)
                        + " in "
                        + Asm4.labelName(rootContainer)
                        + " on:\n\n"
                    )
                ...
                # selected datum is attached
                    ...
                    message = (
                        ...
                        + " in "
                        + Asm4.labelName(rootContainer)
                        + " is already attached to some geometry. "
                    )
    ...
    def getDatumExpression(self, selTree):
        ...
        if (
            firstObj.isDerivedFrom("App::Link")
            and firstObj.LinkedObject.Document != App.ActiveDocument
        ):
        ...
        for objName in selTree[first + 1 :]:
            ...
            if (
                ...
                and obj.LinkedObject.Document != App.ActiveDocument
            ):
