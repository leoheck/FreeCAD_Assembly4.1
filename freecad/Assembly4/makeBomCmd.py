...
class makeBOM:
    ...
    def indent(self, level, tag="|"):
        spaces = (level + 1) * "  "
        return "[{level}]{spaces}{tag} ".format(
            level=str(level), tag=tag, spaces=spaces
        )
    ...
    def RecursivelyAutoFillPartInfoInBom(self, obj, level=0, parent=None):
        ...
        if obj.TypeId == "App::Link":
            ...
            if level <= max_level:
                if (
                    BomKeyAF in self.BomKeyForAutoFillList
                ):  # we don't need to add info because we did it already
        ...
        # ============================
        elif obj.TypeId == "Part::FeaturePython" and (
            obj.Content.find("FastenersCmd") or (obj.Content.find("PCBStandoff")) > -1
        ):
    ...
    def listParts(self, obj, level=0, parent=None):
        ...
        if obj.TypeId == "App::Link":
            if obj.Visibility == True:
                ...
                print(
                    "ASM4> {level}{obj_typeid} | {obj_name} | {obj_label}".format(
                        level=self.indent(level),
                        obj_label=obj.Label,
                        obj_name=obj.FullName,
                        obj_typeid=obj.TypeId,
                    )
                )
            ...
            if level <= max_level:
                ...
                if BomKey in self.BomKeyList:
                    if self.BomKeyList[BomKey]["BomKey"] == BomKey:
                        ...
                        print(
                            "ASM4> {level}{qtd}x | {obj_typeid} | {obj_name} | {obj_label}".format(
                                level=self.indent(level, tag=" "),
                                obj_label=obj.Label,
                                obj_name=obj.FullName,
                                obj_typeid=obj.TypeId,
                                qtd=qtd,
                    ...
                    print(
                        "ASM4> {level}1x | {obj_typeid} | {obj_name} | {obj_label}".format(
                            level=self.indent(level, tag=" "),
                            obj_label=obj.Label,
                            obj_name=obj.FullName,
                            obj_typeid=obj.TypeId,
                    ...
                    for prop in self.infoKeysUser:
                        ...
                        self.BomKeyList[BomKey][
                            self.infoKeysUser.get(prop).get("userData")
                        ] = data
    ...
    def inSpreadsheet(self):
        ...
        def wrow(drow: [str], row: int):
            for i, d in enumerate(drow):
                if row == 0:
                    spreadsheet.set(
                        str(chr(ord("a") + i)).upper() + str(row + 1),
                        infoPartCmd.decodeXml(str(d)),
                    )
                else:
                    spreadsheet.set(
    ...
    def drawUI(self):
        ...
        self.LabelBOML1.setText(
            "BOM generates bill of materials.\n\nIt uses the Parts' info to generate entries on BOM, unless autofill is set.\n"
        )
        self.LabelBOML2 = QtGui.QLabel()
        self.LabelBOML2.setText(
            "Check <a href='https://github.com/Zolko-123/FreeCAD_Assembly4/tree/master/Examples/ConfigBOM/README.md'>BOM tutorial</a>"
        )
