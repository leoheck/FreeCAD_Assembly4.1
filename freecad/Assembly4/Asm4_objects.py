...
class ViewProviderVariant(object):
    ...
    def getIcon(self):
        ...
        if (
            hasattr(self.Object.Type, "Type")
            and self.Object.Type == "Asm4::VariantLink"
        ):
            iconPath = os.path.join(Asm4.iconPath, "Variant_Link.svg")
        return iconPath
...
class LinkArray(object):
    ...
    def attach(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLink", "SourceObject", "Array", "The object to array"
        )
    ...
    def onDocumentRestored(self, obj):
        ...
        if not hasattr(obj, "Placer"):
            ...
            if hasattr(obj, "ElementPlacement"):
                ...
                def pnrepl(s):
                    return s.replace("ElementPlacement", "Placer")
    ...
    def execute(self, obj):
        ...
        if obj.Axis:
            if parent != obj.Axis[0].getParentGeoFeatureGroup():
                self.raiseError(
                    obj, "Source Object and Axis must have the same parent Part"
                )
        ...
        # Fix lighting when mirroing Bodies
        if hasattr(obj.SourceObject, "ViewObject"):
            vo = obj.SourceObject.ViewObject
        ...
        if sub:
            ...
            if Asm4.isCircle(sub):
                return App.Placement(
                    sub.Curve.Center, App.Rotation(Asm4.VEC_Z, sub.Curve.Axis)
                )
                ...
                if _findParam(n, exDict[edge]):
                    if edge not in resolved:
                        if edge in unresolved:
                            raise RuntimeError(
                                "Circular reference detected: {} -> {}".format(n, edge)
                            )
