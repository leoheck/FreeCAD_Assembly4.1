...
class checkInterference:
    ...
    def log_number_of_objects(self):
        ...
        for obj in self.Assembly.Group:
            if obj.Visibility == True and (
                obj.TypeId == "App::Link"
                or (
                    obj.TypeId == "Part::FeaturePython"
                    and (
                        obj.Content.find("FastenersCmd")
                        or (obj.Content.find("PCBStandoff")) > -1
                    )
                )
            ):
                self.n_objects += 1
        ...
        if self.check_fasteners:
            self.log_write(
                "The {} has {} objects. (with fasteners)".format(
                    self.Assembly.Label, self.n_objects
                )
            )
        else:
            self.log_write(
                "The {} has {} objects.".format(
    ...
    def log_number_of_comparisons(self):
        ...
        if self.n_objects > 2:
            self.total_comparisons = (int)(
                math.factorial(self.n_objects)
                / (math.factorial(2) * math.factorial(self.n_objects - 2))
            )
        else:
            self.total_comparisons = 0

        if self.n_objects_without_fasteners > 2:
            self.total_comparisons_without_fasteners = (int)(
                math.factorial(self.n_objects_without_fasteners)
                / (
                    math.factorial(2)
                    * math.factorial(self.n_objects_without_fasteners - 2)
                )
            )
        else:
        ...
        if self.check_fasteners:
            self.log_write(
                "Totaling {} possible comparisons (with fasteners)".format(
                    self.total_comparisons
                )
            )
        else:
            self.log_write(
                "Totaling {} possible comparisons".format(
    ...
    def check_interferences(self):
        ...
        for obj1 in self.Assembly.Group:
            ...
            if obj1.Visibility == True and (
                ...
                or (
                    self.check_fasteners
                    and (
                        obj1.TypeId == "Part::FeaturePython"
                        and (
                            obj1.Content.find("FastenersCmd")
                            or (obj1.Content.find("PCBStandoff")) > -1
                        )
                    )
                )
            ):
                    ...
                    if obj2 != obj1:
                        ...
                        if obj2.Visibility == True and (
                            ...
                            or (
                                self.check_fasteners
                                and (
                                    obj2.TypeId == "Part::FeaturePython"
                                    and (
                                        obj2.Content.find("FastenersCmd")
                                        or (obj2.Content.find("PCBStandoff")) > -1
                                    )
                                )
                            )
                        ):
                            ...
                            if not obj2.Label in checked_dict[obj1.Label]:
                                ...
                                self.log_write(
                                    "\n{count}. {obj1} vs {obj2}".format(
                                        count=str(c).rjust(indent),
                                        obj1=obj1.Label,
                                        obj2=obj2.Label,
                                    )
                                )
                                ...
                                if self.verbose:
                                    self.log_write(
                                        "{count: {width}}. {obj1} vs {obj2}".format(
                                            count=c,
                                            width=self.width_of_number_of_objects(),
                                            obj1=obj1.Label,
                                            obj2=obj2.Label,
                                        )
                                    )
                                    indent = " " * (
                                        self.width_of_number_of_objects() + 2
                                    )
                                    self.log_write(
        ...
        if not Intersections.Group:
            self.log_write(
                "\n>>> No interferences were found. The {} is clean! <<<".format(
                    self.Assembly.Label
                )
            )
    ...
    def make_shape_copy(self, doc, obj):
        ...
        new_obj.ViewObject.ShapeColor = getattr(
            obj.getLinkedObject(True).ViewObject,
            "ShapeColor",
            new_obj.ViewObject.ShapeColor,
        )
        new_obj.ViewObject.LineColor = getattr(
            obj.getLinkedObject(True).ViewObject,
            "LineColor",
            new_obj.ViewObject.LineColor,
        )
        new_obj.ViewObject.PointColor = getattr(
            obj.getLinkedObject(True).ViewObject,
            "PointColor",
            new_obj.ViewObject.PointColor,
        )
        return new_obj
    ...
    def make_part_intersection(self, doc, obj1, obj2, count):
        ...
        obj.ViewObject.DisplayMode = getattr(
            obj1.getLinkedObject(True).ViewObject,
            "DisplayMode",
            obj.ViewObject.DisplayMode,
        )
        obj.ViewObject.ShapeColor = (1.0, 0.666, 0.0)  # YELLOW
    ...
    def on_check(self):
        ...
        self.log_write(
            "Minimum interference volume = {}".format(self.min_volume_allowed)
        )
    ...
    def drawUI(self):
        ...
        self.log_view.setStyleSheet(
            "QTextEdit:!editable{color: white, background-color: black};"
        )
