...
class openConfigurationsCmd:
    ...
    def GetResources(self):
        return {
            "MenuText": "Open configurations panel",
        ...
        if len(selectedItems) == 1:
            ...
            if isAsm4Config(conf):
                confirm = Asm4.confirmBox(
                    'This will delete configuration "' + confName + '"?'
                )
                if confirm:
...
def SaveSubObjects(conf, container):
    for objName in container.getSubObjects():
        ...
        # save subobjects in groups, but not the groups themselves. Skip default Asm4 groups
            ...
            and obj.Name != "Configurations"
            and obj.Name != "Constraints"
            and obj.Name != "Measures"
        ):
...
def SaveObject(conf, obj):
    ...
    if (
        ...
        or obj.TypeId == "Part::FeaturePython"
    ):
        SaveSubObjects(conf, obj)
    ...
    # if it's an Assembly4 object
        ...
        asmType == "Asm4EE"
        or asmType == "Part::Link"
        or asmType == "Placement::ExpressionEngine"
    ):
        asmType = "Asm4EE"
        conf.set(OBJECT_ASM_TYPE_COL + row, str(asmType))
